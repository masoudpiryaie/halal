...
import {
  MembershipAgentTypeCreateDialog,
  MembershipAgentTypeModel,
} from './MembershipAgentTypeCreateDialog'
...
export function MembershipAgentTypePage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('MembershipAgentTypeGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<MembershipAgentTypeModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'MembershipAgentType/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.MembershipAgentTypeController'})}
      </PageTitle>

      {showDialog && (
        <MembershipAgentTypeCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'MembershipAgentType/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='MembershipAgentTypeGrid'
