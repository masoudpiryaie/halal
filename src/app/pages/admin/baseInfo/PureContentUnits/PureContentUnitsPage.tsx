...
import {PureContentUnitsCreateDialog, PureContentUnitsModel} from './PureContentUnitsCreateDialog'
...
export function PureContentUnitsPage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('PureContentUnitsGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<PureContentUnitsModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'PureContentUnits/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.PureContentUnitsController'})}
      </PageTitle>

      {showDialog && (
        <PureContentUnitsCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'PureContentUnits/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='PureContentUnitsGrid'
                  exteraParam={'&PureContentUnit=' + 1}
                >
                  <GridColumn
                    filterable={false}
                    title='فعال/غیر فعال'
