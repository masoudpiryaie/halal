...
import {BaseNamesCreateDialog, BaseNamesModel} from './BaseNamesCreateDialog'
import {SearchFunc} from 'app/pages/admin/baseInfo/SearchFunc'
import axios from 'axios'
import {StringSchema} from 'yup'

export function BaseNamesPage() {
  const intl = useIntl()
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.BaseNamesController'})}
      </PageTitle>
      <PageActions>
      ...
      {showDialog && <BaseNamesCreateDialog mode={mode} onClose={handleClose} item={actionItem} />}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URLCoding + 'baseNames'}
                  showActions
                ...
                >
                  <GridColumn
                    field='titleFa'
                    title={intl.formatMessage({id: 'Model.BaseNames.TitleFa'})}
                  />
                  <GridColumn
                    field='code'
                    title={intl.formatMessage({id: 'Model.BaseNames.Code'})}
                  />
                </Grid>
              </div>
            </div>
