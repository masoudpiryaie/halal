...
import {ServicesCreateDialog, ServicesModel} from './ServicesCreateDialog'
...
export function ServicesPage() {
  ...
  const {
    ...
    reset,
    formState: {isDirty},
  } = useForm<ServicesModel>()
  const [filter, setFilter] = useState<CompositeFilterDescriptor>()
  const gr = localStorage.getItem('Services')
  var col = gr
    ? (JSON.parse(gr) as IModelGr[])
    : [
        {
          title: intl.formatMessage({id: 'Model.Services.Title'}),
          field: 'title',
          show: true,
        },
      ]
  ...
  const searchItems = [
    intl.formatMessage({id: 'Model.Services.Title'}) + '*title-0',
    intl.formatMessage({id: 'Model.Services.IsEnabled'}) + '*isEnabled-1',
  ]
  const ToggleEnable = async (data: number) => {
    let url = API_URL + 'services/ToggleEnable/' + data
    try {
      await axios.put(url)
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.ServicesController'})}
      </PageTitle>
      <PageActions>
        <BtnAction onClick={newItem} label={intl.formatMessage({id: 'MENU.NewItem'})} />
      </PageActions>
      {showDialog && <ServicesCreateDialog mode={mode} onClose={handleClose} item={actionItem} />}
