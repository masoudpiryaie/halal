...
import {
  MembershipAddressTypeCreateDialog,
  MembershipAddressTypeModel,
} from './MembershipAddressTypeCreateDialog'
...
export function MembershipAddressTypePage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('MembershipAddressTypeGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<MembershipAddressTypeModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'MembershipAddressType/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.MembershipAddressTypeController'})}
      </PageTitle>

      {showDialog && (
        <MembershipAddressTypeCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'MembershipAddressType/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='MembershipAddressTypeGrid'
