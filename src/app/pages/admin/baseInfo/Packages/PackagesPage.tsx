...
import {PackagesCreateDialog, PackagesModel} from './PackagesCreateDialog'
...
export function PackagesPage() {
  ...
  const gr = localStorage.getItem('PackagesGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<PackagesModel>()
  ...
  const hanleActions = async (action: string, item: any) => {
    if (action === 'edit') {
      try {
        let url = API_URL + 'Packages/' + item?.id
  ...
  const searchItems = ['عنوان*title-0', 'نوع*type-10']
  var col = gr
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'Packages/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.PackagesController'})}
      </PageTitle>

      {showDialog && <PackagesCreateDialog mode={mode} onClose={handleClose} item={actionItem} />}
