...
import {ViolationTitleCreateDialog, ViolationTitleModel} from './ViolationTitleCreateDialog'
import {SearchFunc} from 'app/pages/admin/baseInfo/SearchFunc'
import {SearchLayout, GridLayout, PageLayout} from '@core/layout'

export function ViolationTitlePage() {
  const intl = useIntl()
  ...
  const searchItems = [
    intl.formatMessage({id: 'Model.ViolationTitle.Name'}) + '*name-0',
    intl.formatMessage({id: 'Model.ViolationTitle.IsActive'}) + '*isActive-1',
    intl.formatMessage({id: 'Model.ViolationTitle.CountryName'}) + '*countryId-2',
  ]
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.ViolationTitleController'})}
      </PageTitle>
      <PageActions>
      ...
      {showDialog && (
        <ViolationTitleCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
      <PageLayout>
        ...
        <GridLayout>
          ...
          >
            <GridColumn
              field='name'
              title={intl.formatMessage({id: 'Model.ViolationTitle.Name'})}
            />
            <GridColumn
              title={intl.formatMessage({id: 'Model.ViolationTitle.Type'})}
              field='type'
              cell={(props) => (
                <td>
                  <label>
                    {intl.formatMessage({
                      id: 'FormControl.Input.Select.ViolationTitleType' + props.dataItem.type,
                    })}
                  </label>
                </td>
              )}
            ...
            <GridColumn
              field='countryName'
              title={intl.formatMessage({id: 'Model.ViolationTitle.CountryName'})}
            />
            <GridColumn
              title={intl.formatMessage({id: 'Model.ViolationTitle.IsActive'})}
              field='isActive'
              filter='boolean'
              cell={(props) => (
                <td>
