...
export function ShipmentCreateDialog({mode, onClose, item}: PropsType) {
  const intl = useIntl()
  const {
    register,
    handleSubmit,
  ...
  } = useForm<ShipmentModel>({mode: 'all'})
      ...
      <Dialog title={pageTitles[mode]} onClose={onClose}>
        <div>
          <form id='dialog-form' onSubmit={onSubmit} className='row g-3'>
            ...
            <FormControl col={12} label='email'>
              <input
                className={getValidationClass(touchedFields.name, errors.name)}
                autoFocus={true}
            ...
            <FormControl col={12} label='cNPJ'>
              <input
                className={getValidationClass(touchedFields.name, errors.name)}
                autoFocus={true}
                {...register('cNPJ', {required: true, minLength: 2})}
              />
            </FormControl>
          </form>
        </div>
