...
import {TariffsCreateDialog, TariffsModel} from './TariffsCreateDialog'
...
export function TariffsPage() {
  ...
  const {
    ...
    reset,
    formState: {isDirty},
  } = useForm<TariffsModel>()
  const [filter, setFilter] = useState<CompositeFilterDescriptor>()
  const gr = localStorage.getItem('Tariffs')
  ...
  const [inputList, setInputList] = useState([
    {field: 'productId', value: '', operator: 'eq', type: '2'},
  ])
  ...
  const handleAddClick = () => {
    if (inputList.filter((x) => x.value === '').length === 0)
      setInputList([...inputList, {field: 'productId', value: '', operator: 'eq', type: '2'}])
  }
  const searchItems = ['محصول*productId-2']
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.TariffsController'})}
      </PageTitle>

      {showDialog && <TariffsCreateDialog mode={mode} onClose={handleClose} item={actionItem} />}
