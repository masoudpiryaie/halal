...
export function IranCodeStructurePage() {
  const intl = useIntl()
  ...
  const fetchSTree = async () => {
    try {
      let url = API_URL + 'IRCodeStructures?SortDirection=asc'
  ...
  const delTree = async (id: any) => {
    try {
      let url = API_URL + 'IRCodeStructures/' + id
  ...
  const ActiveTree = async (id: any) => {
    try {
      ...
      setActiveID(id)
      setURLActive(API_URL + 'IRCodeStructures/')
  ...
  const PayeInfosActive = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'PayeInfos/' + id
      const resp = await axios.get(url)
      setstate(resp.data.value.isEnabled)
      setActiveID(id)
      setURLActive(API_URL + 'PayeInfos/')
      setshowActiveDialog(true)
    } catch (error) {
      console.log(error)
    }
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.IranCodeStructureController'})}
      </PageTitle>
      {deleteConfirm && (
      ...
      {/* {showDialog && (
        <IranCodeStructureCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )} */}
      {showTreeDialog && (
        <TreeCreateDialog
          mode={mode}
      ...
      <div className='card card-flush '>
        <div className='card-body'>
          ...
          <div className='row g-0'>
            <div className='col-3 card-px-0 mb-2 shadow '>
              <div
                style={{
          ...
          <div className='row g-0'>
            <div className='col-3 card-px-0 mb-2 shadow overflow-scroll'>
              <div
                style={{
                  height: '10px',
                }}
            ...
            <div className='col-9'>
              <Grid
                ...
                exteraParam={'&IRCodeStructureId=' + treeID}
              >
                <GridColumn
                  filterable={false}
                  title='فعال/غیر فعال'
