...
import {EntityFilesCreateDialog, EntityFilesModel} from './EntityFilesCreateDialog'
...
export function EntityFilesPage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('EntityFilesGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<EntityFilesModel>()
  ...
  const [inputList, setInputList] = useState([
    {field: 'groupTitle', value: '', operator: 'eq', type: '0'},
  ])
  ...
  const handleAddClick = () => {
    if (inputList.filter((x) => x.value === '').length === 0)
      setInputList([...inputList, {field: 'groupTitle', value: '', operator: 'eq', type: '0'}])
  }
  const searchItems = [
    intl.formatMessage({id: 'Model.EntityFile.GroupTitle'}) + '*groupTitle-0',
    intl.formatMessage({id: 'Model.EntityFile.FileType'}) + '*fileType-0',

    // intl.formatMessage({id: 'Model.EntityFile.IsEnabled'}) + '*isEnabled-1',
  ]
  var col = gr
    ? (JSON.parse(gr) as IModelGr[])
    : [
        {
          title: intl.formatMessage({id: 'Model.EntityFile.GroupTitle'}),
          field: 'groupTitle',
          show: true,
        },
        {
          title: intl.formatMessage({id: 'Model.EntityFile.FileType'}),
          field: 'fileType',
          show: true,
        },
        {
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'EntityFiles/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.EntityFileController'})}
      </PageTitle>

      {showDialog && (
        <EntityFilesCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'EntityFiles/'}
                  showDel
                  showEdit={false}
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='EntityFilesGrid'
                >
                  {/* <GridColumn
                    filterable={false}
                    title='فعال/غیر فعال'
