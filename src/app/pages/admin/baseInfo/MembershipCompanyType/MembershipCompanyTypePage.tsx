...
import {
  MembershipCompanyTypeCreateDialog,
  MembershipCompanyTypeModel,
} from './MembershipCompanyTypeCreateDialog'
...
export function MembershipCompanyTypePage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('MembershipCompanyTypeGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<MembershipCompanyTypeModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'MembershipCompanyType/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.MembershipCompanyTypeController'})}
      </PageTitle>

      {showDialog && (
        <MembershipCompanyTypeCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'MembershipCompanyType/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='MembershipCompanyTypeGrid'
