...
export function ShenaseStructurePage() {
  const intl = useIntl()
  ...
  const fetchSTree = async () => {
    try {
      let url = API_URL + 'ShenaseStructures?SortDirection=asc'
  ...
  const delTree = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'ShenaseStructures/' + id
  ...
  const ActiveTree = async (id: any) => {
    try {
      ...
      setActiveID(id)
      setURLActive(API_URL + 'ShenaseStructures/')
  ...
  const PayeInfosActive = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'HeadLines/' + id
      const resp = await axios.get(url)
      setstate(resp.data.value.isEnabled)
      setActiveID(id)
      setURLActive(API_URL + 'HeadLines/')
      setshowActiveDialog(true)
    } catch (error) {
      console.log(error)
    }
          ...
          <div className='row g-0'>
            ...
            <div className='col-8'>
              <Grid
                ...
                columns={col}
                name='payeInfo'
                exteraParam={'&ShenaseStructureId=' + treeID}
              >
                <GridColumn
                  filterable={false}
                  title='فعال/غیر فعال'
