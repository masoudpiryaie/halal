...
import {AddressTypeCreateDialog, AddressTypeModel} from './AddressTypeCreateDialog'
...
export function AddressTypePage() {
  const intl = useIntl()
  ...
  const {
    ...
    reset,
    formState: {isDirty},
  } = useForm<AddressTypeModel>()
  const [filter, setFilter] = useState<CompositeFilterDescriptor>()
  const gr = localStorage.getItem('addressType')
  ...
  const searchItems = [
    intl.formatMessage({id: 'Model.AddressType.TitleFa'}) + '*titleFa-0',
    intl.formatMessage({id: 'Model.AddressType.IsEnabled'}) + '*isEnabled-1',
  ]
  const ToggleEnable = async (data: number) => {
    let url = API_URL + 'addressTitle/ToggleEnable/' + data
    try {
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.AddressTypeController'})}
      </PageTitle>

      {showDialog && (
        <AddressTypeCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
