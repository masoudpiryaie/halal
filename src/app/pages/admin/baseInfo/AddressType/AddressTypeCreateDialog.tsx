...
export function AddressTypeCreateDialog({mode, onClose, item}: PropsType) {
  const intl = useIntl()
  ...
  } = useForm<AddressTypeModel>({mode: 'all'})
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      if (mode !== 'edit') {
        let url = API_URLCoding + 'addressTitle'
        await axios.post(url, data)
      } else {
        let url = API_URLCoding + 'addressTitle/UpdateJson'
      ...
      <Dialog title={pageTitles[mode]} onClose={onClose}>
        <div>
          <form id='dialog-form' onSubmit={onSubmit} className='row g-3'>
            <FormControl col={12} label={intl.formatMessage({id: 'Model.AddressType.TitleFa'})}>
              <input
            ...
            <FormControl col={12} label={intl.formatMessage({id: 'Model.AddressType.TitleEn'})}>
              <input
