...
import {UnitOfWeightsCreateDialog, UnitOfWeightsModel} from './UnitOfWeightsCreateDialog'
...
export function UnitOfWeightsPage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('UnitOfWeightsGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<UnitOfWeightsModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'UnitOfWeights/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.UnitOfWeightsController'})}
      </PageTitle>

      {showDialog && (
        <UnitOfWeightsCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'UnitOfWeights/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='UnitOfWeightsGrid'
