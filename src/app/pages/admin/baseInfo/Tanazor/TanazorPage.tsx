...
import {TanazorCreateDialog, BreakheadLineModel} from './TanazorCreateDialog'
...
export function TanazorPage() {
  ...
  const PayeInfosActive = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'BreakHeadLines/' + id
      const resp = await axios.get(url)
      setstate(resp.data.value.isEnabled)
      setActiveID(id)
      setURLActive(API_URL + 'BreakHeadLines/')
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.TanazorController'})}
      </PageTitle>
      <PageActions>
        <BtnAction onClick={newItem} label={intl.formatMessage({id: 'MENU.NewItem'})} />
      </PageActions>
      {showDialog && <TanazorCreateDialog mode={mode} onClose={handleClose} item={actionItem} />}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'BreakHeadLines'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='BreakHeadLinesGrid'
                >
                  <GridColumn
                    filterable={false}
                    title='فعال/غیر فعال'
