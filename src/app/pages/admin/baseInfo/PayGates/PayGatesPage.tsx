...
import {PayGatesCreateDialog, PayGatesModel} from './PayGatesCreateDialog'
...
export function PayGatesPage() {
  ...
  const gr = localStorage.getItem('PayGatesGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<PayGatesModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'PayGates/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.PayGatesController'})}
      </PageTitle>

      {showDialog && <PayGatesCreateDialog mode={mode} onClose={handleClose} item={actionItem} />}
