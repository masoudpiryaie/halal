...
import {
  MembershipActivityTypeCreateDialog,
  MembershipActivityTypeModel,
} from './MembershipActivityTypeCreateDialog'
...
export function MembershipActivityTypePage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('MembershipActivityTypeGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<MembershipActivityTypeModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'MembershipActivityType/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.MembershipActivityTypeController'})}
      </PageTitle>

      {showDialog && (
        <MembershipActivityTypeCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'MembershipActivityType/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='MembershipActivityTypeGrid'
