...
type PropsType = {
  mode: FormMode
  onClose: () => void
  item?: MembershipActivityTypeModel
}
export type MembershipActivityTypeModel = {
  id?: number
  titleFa: string
  titleEn: string
}
...
export function MembershipActivityTypeCreateDialog({mode, onClose, item}: PropsType) {
  const intl = useIntl()
  ...
  } = useForm<MembershipActivityTypeModel>({mode: 'all'})
  ...
  const onSubmit = handleSubmit(async (data) => {
    let url = API_URL + 'MembershipActivityType'
