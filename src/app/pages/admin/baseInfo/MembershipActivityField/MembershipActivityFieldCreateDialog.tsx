...
type PropsType = {
  mode: FormMode
  onClose: () => void
  item?: MembershipActivityFieldModel
}
export type MembershipActivityFieldModel = {
  id?: number
  titleFa: string
  titleEn: string
}
...
export function MembershipActivityFieldCreateDialog({mode, onClose, item}: PropsType) {
  const intl = useIntl()
  ...
  } = useForm<MembershipActivityFieldModel>({mode: 'all'})
  ...
  const onSubmit = handleSubmit(async (data) => {
    let url = API_URL + 'MembershipActivityField'
