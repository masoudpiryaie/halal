...
import {
  MembershipActivityFieldCreateDialog,
  MembershipActivityFieldModel,
} from './MembershipActivityFieldCreateDialog'
...
export function MembershipActivityFieldPage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('LanguagesGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<MembershipActivityFieldModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'MembershipActivityField/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.MembershipActivityFieldController'})}
      </PageTitle>

      {showDialog && (
        <MembershipActivityFieldCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'MembershipActivityField/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='MembershipActivityFieldGrid'
