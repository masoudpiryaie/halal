...
import {
  UnitOfMeasurementsCreateDialog,
  UnitOfMeasurementsModel,
} from './UnitOfMeasurementsCreateDialog'
...
export function UnitOfMeasurementsPage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('UnitOfMeasurementsGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<UnitOfMeasurementsModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'UnitOfMeasurements/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.UnitOfMeasurementsController'})}
      </PageTitle>

      {showDialog && (
        <UnitOfMeasurementsCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'UnitOfMeasurements/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='UnitOfMeasurementsGrid'
