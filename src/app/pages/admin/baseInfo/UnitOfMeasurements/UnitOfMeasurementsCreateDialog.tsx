...
type PropsType = {
  mode: FormMode
  onClose: () => void
  item?: UnitOfMeasurementsModel
}
export type UnitOfMeasurementsModel = {
  id?: number
  titleFa: string
  titleEn: string
}
...
export function UnitOfMeasurementsCreateDialog({mode, onClose, item}: PropsType) {
  const intl = useIntl()
  ...
  } = useForm<UnitOfMeasurementsModel>({mode: 'all'})
  ...
  const onSubmit = handleSubmit(async (data) => {
    let url = API_URL + 'UnitOfMeasurements'
