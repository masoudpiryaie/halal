...
import {ContactInfoCreateDialog, ContactInfoModel} from './ContactInfoCreateDialog'
...
export function ContactInfoPage() {
  ...
  const gr = localStorage.getItem('ContactInfoGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<ContactInfoModel>()
  ...
  const searchItems = [
    intl.formatMessage({id: 'Model.PureContentUnits.TitleFa'}) + '*addressFa-0',
    intl.formatMessage({id: 'Model.PureContentUnits.TitleEn'}) + '*addressEn-0',
  ]
  var col = gr
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'ContactInfo/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.ContactInfoController'})}
      </PageTitle>

      {showDialog && (
        <ContactInfoCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
