...
import {TargetMarketsCreateDialog, TargetMarketsModel} from './TargetMarketsCreateDialog'
...
export function TargetMarketsPage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('TargetMarketsGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<TargetMarketsModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'TargetMarkets/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.TargetMarketsController'})}
      </PageTitle>

      {showDialog && (
        <TargetMarketsCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'TargetMarkets/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='TargetMarketsGrid'
                >
                  <GridColumn
                    filterable={false}
                    title='فعال/غیر فعال'
