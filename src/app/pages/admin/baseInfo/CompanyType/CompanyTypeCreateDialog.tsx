...
export function CompanyTypeCreateDialog({mode, onClose, item}: PropsType) {
  const intl = useIntl()
  ...
  } = useForm<CompanyTypeModel>({mode: 'all'})
  ...
  const onSubmit = handleSubmit(async (data) => {
    debugger
    data.isEnabled = true
    let url = API_URLCoding + 'companyType'

    try {
      if (mode !== 'edit') await axios.post(url, data)
      else await axios.put(url, {title: data.titleFa, isEnabled: true, id: data.id})
      ...
      <Dialog title={pageTitles[mode]} onClose={onClose}>
        <div>
          <form id='dialog-form' onSubmit={onSubmit} className='row g-3'>
            <FormControl col={12} label={intl.formatMessage({id: 'Model.CompanyType.TitleFa'})}>
              <input
            ...
            <FormControl col={12} label={intl.formatMessage({id: 'Model.CompanyType.TitleEn'})}>
              <input
