...
type PropsType = {
  mode: FormMode
  onClose: () => void
  item?: BreakPayeInfoModel
}
export type BreakPayeInfoModel = {
  id?: number
  breakId: number
  payeInfoIds: number[]
}
...
export function Tanazor1CreateDialog({mode, onClose, item}: PropsType) {
  const intl = useIntl()
  ...
  } = useForm<BreakPayeInfoModel>({mode: 'all'})
  ...
  const onSubmit = handleSubmit(async (data) => {
    let url = API_URL + 'BreakPayeInfos'
    if (breakGridID === 0) {
      alert('انتخاب breakاجباری است')
    } else {
      try {
        data.payeInfoIds = checkBoxData
