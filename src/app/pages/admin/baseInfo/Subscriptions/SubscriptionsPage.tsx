...
import {SubscriptionsCreateDialog, SubscriptionsModel} from './SubscriptionsCreateDialog'
...
export function SubscriptionsPage() {
  ...
  const gr = localStorage.getItem('SubscriptionsGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<SubscriptionsModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'Subscriptions/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.SubscriptionsController'})}
      </PageTitle>

      {showDialog && (
        <SubscriptionsCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
