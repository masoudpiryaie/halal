import {GridColumn} from '@progress/kendo-react-grid'
import {FormControl, Grid, CheckBoxAction, BtnAction} from '@core/components'
import {useState} from 'react'
import {useIntl} from 'react-intl'
import {FormMode} from '@core/forms'
import {PageTitle, PageActions, useLayout} from '_metronic/layout/core'
import {useForm} from 'react-hook-form'
import {CompositeFilterDescriptor} from '@progress/kendo-data-query'
import {generateFilter, generateFiltereq} from '@core/utils'
import {ProductsCreateDialog, ProductsModel} from './ProductsCreateDialog'
import {SearchFunc} from 'app/pages/admin/baseInfo/SearchFunc'
import {ActiveDialog} from 'app/pages/admin/baseInfo/Component/ActiveDialog'
import {TariffsCreateDialog} from './TariffsCreateDialog'
import axios from 'axios'
import {StringSchema} from 'yup'
import {toast} from 'react-toastify'
export interface IModelGr {
  title: string
  field: string
  show: boolean
}
export function ProductsPage() {
  const intl = useIntl()
  const [searchShow, setsearchShow] = useState(true)
  const {config} = useLayout()
  const API_URL = process.env.REACT_APP_API_URLCoding || 'api'
  const [showDialog, setShowDialog] = useState(false)
  const [mode, setMode] = useState<FormMode>('new')
  const [actionItem, setActionItem] = useState()
  const [refresh, setRefresh] = useState(1)
  const [showActiveDialog, setshowActiveDialog] = useState(false)
  const [showStateDialog, setshowStateDialog] = useState(false)
  const [filter1, setFilter1] = useState<CompositeFilterDescriptor>()
  const [refreshprov, setrefreshprov] = useState(1)
  const [provinceList, setprovinceList] = useState('tariffs')
  const [countryName, setCountryName] = useState('')
  const [actionproductId, setActionproductId] = useState(0)
  const [actionStateItem, setActionStateItem] = useState()

  const gr = localStorage.getItem('ProductsGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<ProductsModel>()
  const [filter, setFilter] = useState<CompositeFilterDescriptor>()

  const onSubmit = handleSubmit(async (data) => {
    setFilter({logic: 'and', filters: inputList})
  })
  const resetSearch = () => {
    reset()
    setFilter(undefined)
  }
  const handleClose = () => {
    setShowDialog(false)
    setRefresh(Math.random())
  }
  const newItem = () => {
    setMode('new')
    setShowDialog(true)
  }
  const hanleActions = async (action: string, item: any) => {
    if (action === 'edit') {
      try {
        let url = API_URL + 'Products/' + item?.id

        const resp = await axios.get(url)
        debugger
        setActionItem(resp.data.value)
        setMode('edit')
        setShowDialog(true)
      } catch (error) {
        console.log(error)
      }
    }
    if (action === 'changeState') {
      Active(item.id)
    }
  }

  const [inputList, setInputList] = useState([
    {field: 'type', value: '1', operator: 'eq', type: '10'},
  ])

  const handleInputChange = (e: any, index: any) => {
    const {name, value} = e.target
    const list = [...inputList]
    if (name === 'field') {
      list[index].field = value.split('-')[0]
      list[index].type = value.split('-')[1]
    } else if (name === 'operator') list[index].operator = value
    else list[index].value = value
    setInputList(list)
  }
  const handleRemoveClick = (index: any) => {
    if (inputList.length !== 1) {
      const list = [...inputList]
      list.splice(index, 1)
      setInputList(list)
    }
  }

  const handleAddClick = () => {
    if (inputList.filter((x) => x.value === '').length === 0)
      setInputList([...inputList, {field: 'type', value: '', operator: 'eq', type: '10'}])
  }
  const searchItems = ['گروه بسته*type-10', 'نوع*type-11']
  var col = gr
    ? (JSON.parse(gr) as IModelGr[])
    : [
        {
          title: 'گروه بسته',
          field: 'packageType',
          show: true,
        },
        {
          title: 'نوع محصول',
          field: 'type',
          show: true,
        },

        {
          title: 'عنوان',
          field: 'title',
          show: true,
        },
      ]
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'Products/ToggleEnable/' + id
      await axios.post(url)
      setRefresh(Math.random())
      toast.info(intl.formatMessage({id: 'MENU.ACTIONSuccess'}), {
        position: 'top-center',
      })
    } catch (error) {
      console.log(error)
    }
  }
  const clickRow = (item: any) => {
    debugger
    setActionproductId(item.dataItem.id)
    setCountryName(item.dataItem.name)
    setFilter1(generateFiltereq({ProductId: item.dataItem.id}))
    setprovinceList(API_URL + 'Tariffs/')

    debugger
  }
  const newItemState = () => {
    if (actionproductId !== 0) {
      setActionStateItem(actionStateItem)
      setMode('new')
      setshowStateDialog(true)
    } else
      toast.info('یک سطر را انتخاب کنید', {
        position: 'top-center',
      })
  }
  const grtariff = localStorage.getItem('tariff')
  var col1 = grtariff
    ? (JSON.parse(grtariff) as IModelGr[])
    : [
        {
          title: 'تاریخ ',
          field: 'StartDateFa',
          show: true,
        },
        {
          title: 'قیمت ',
          field: 'price',
          show: true,
        },

        {
          title: 'درصد ',
          field: 'agentPercent',
          show: true,
        },
      ]
  const hanleActionsState = (action: string, item: any) => {
    // if (action === 'edit') {
    //   setActionStateItem(item)
    //   setMode('edit')
    //   setshowStateDialog(true)
    // }
  }
  const handleCloseState = () => {
    setshowStateDialog(false)
    setrefreshprov(Math.random())
  }

  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.ProductsController'})}
      </PageTitle>

      {showDialog && <ProductsCreateDialog mode={mode} onClose={handleClose} item={actionItem} />}

      <div className='card card-flush '>
        <div className='card-body'>
          <div className='row'>
            <div
              style={{
                backgroundColor: config.main?.primaryColor,
                padding: '1rem 1rem',
                borderRadius: '0.475rem',
                color: '#e4e6ef',
                cursor: 'pointer',
                fontFamily: config.main?.font,
              }}
              onClick={() => setsearchShow(false)}
              hidden={!searchShow}
            >
              نمایش پنل جستجو
            </div>
            <div
              style={{
                backgroundColor: config.main?.primaryColor,
                padding: '1rem 1rem',
                borderTopRightRadius: '0.475rem',
                borderTopLeftRadius: '0.475rem',
                color: '#e4e6ef',
                cursor: 'pointer',
                fontFamily: config.main?.font,
              }}
              onClick={() => setsearchShow(true)}
              hidden={searchShow}
            >
              <label> بستن پنل جستجو</label>
            </div>
            <form
              style={{
                borderBottomLeftRadius: '0.475rem',
                borderBottomRightRadius: '0.475rem',
                paddingBottom: '5px',
              }}
              onSubmit={onSubmit}
              className='card-px-0 mb-2 shadow row g-0'
            >
              <div hidden={searchShow}>
                {inputList.map((x, i) => (
                  <SearchFunc
                    x={x}
                    i={i}
                    SearchModel={searchItems}
                    handleRemoveClick={() => handleRemoveClick(i)}
                    handleAddClick={() => handleAddClick()}
                    handleInputChange={(e) => handleInputChange(e, i)}
                  ></SearchFunc>
                ))}
                <div style={{marginTop: '-10px'}} className='col d-flex flex-row-reverse'>
                  <BtnAction onClick={resetSearch} label=' پاک کردن' />
                  <BtnAction onClick={onSubmit} label='جستجو' />
                </div>
              </div>
            </form>
          </div>
          <div className='row'>
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                <BtnAction onClick={newItem} label={intl.formatMessage({id: 'MENU.NewItem'})} />

                <Grid
                  style={{
                    height: '300px',
                  }}
                  refresh={refresh}
                  endpoint={API_URL + 'Products/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  showChangeState={true}
                  filter={filter}
                  columns={col}
                  name='ProductsGrid'
                  onRowClick={clickRow}
                >
                  <GridColumn
                    filterable={false}
                    title='فعال/غیر فعال'
                    field='isEnabled'
                    filter='boolean'
                    cell={(props) => (
                      <td>
                        <label>{!props.dataItem.isEnabled ? 'غیر فعال' : 'فعال'}</label>
                      </td>
                    )}
                  />
                </Grid>
              </div>
              <div className='row'>
                <div className='col-md-12'>
                  <BtnAction
                    onClick={newItemState}
                    label={intl.formatMessage({id: 'MENU.NewItem'})}
                  />

                  {showStateDialog && (
                    <TariffsCreateDialog
                      mode={mode}
                      onClose={handleCloseState}
                      item={actionStateItem}
                      parent={actionproductId}
                    />
                  )}
                  <div className='card-body' style={{padding: '0px'}}>
                    <Grid
                      style={{
                        height: '300px',
                      }}
                      refresh={refreshprov}
                      endpoint={provinceList}
                      showDel
                      showEdit={false}
                      onAction={hanleActionsState}
                      filter={filter1}
                      columns={col1}
                      name='tariff'
                    ></Grid>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  )
}
