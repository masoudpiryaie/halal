...
import {AgentTypesCreateDialog, AgentTypesModel} from './AgentTypesCreateDialog'
...
export function AgentTypesPage() {
  ...
  const {
    ...
    reset,
    formState: {isDirty},
  } = useForm<AgentTypesModel>()
  const [filter, setFilter] = useState<CompositeFilterDescriptor>()
  const gr = localStorage.getItem('agentTypes')

  var col = gr
    ? (JSON.parse(gr) as IModelGr[])
    : [
        {
          title: intl.formatMessage({id: 'Model.AgentTypes.TitleFa'}),
          field: 'titleFa',
          show: true,
        },
        {
          title: intl.formatMessage({id: 'Model.AgentTypes.TitleEn'}),
          field: 'titleEn',
          show: true,
        },
      ]
  ...
  const searchItems = [
    intl.formatMessage({id: 'Model.AgentTypes.TitleFa'}) + '*title-0',
    intl.formatMessage({id: 'Model.AgentTypes.IsEnabled'}) + '*isEnabled-1',
  ]
  const ToggleEnable = async (data: number) => {
    let url = API_URL + 'agentTypes/ToggleEnable/' + data
    try {
      await axios.put(url)
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.AgentTypesController'})}
      </PageTitle>

      {showDialog && <AgentTypesCreateDialog mode={mode} onClose={handleClose} item={actionItem} />}
