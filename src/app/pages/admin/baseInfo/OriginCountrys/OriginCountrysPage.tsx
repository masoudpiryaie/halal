...
import {OriginCountrysCreateDialog, OriginCountrysModel} from './OriginCountrysCreateDialog'
...
export function OriginCountrysPage() {
  const intl = useIntl()
  ...
  const gr = localStorage.getItem('OriginCountrysGrid')
  const {
    register,
    handleSubmit,
    reset,
    formState: {isDirty},
  } = useForm<OriginCountrysModel>()
  ...
  const Active = async (id: any) => {
    try {
      debugger
      let url = API_URL + 'OriginCountrys/ToggleEnable/' + id
  ...
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.BaseInfo.OriginCountrysController'})}
      </PageTitle>

      {showDialog && (
        <OriginCountrysCreateDialog mode={mode} onClose={handleClose} item={actionItem} />
      )}
            ...
            <div className='card card-px-0 shadow'>
              <div className='card-body'>
                ...
                <Grid
                  ...
                  refresh={refresh}
                  endpoint={API_URL + 'OriginCountrys/'}
                  showDel
                  showEdit
                  onAction={hanleActions}
                  filter={filter}
                  columns={col}
                  name='OriginCountrysGrid'
                >
                  <GridColumn
                    filterable={false}
                    title='فعال/غیر فعال'
