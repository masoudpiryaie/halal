...
const registrationSchema = Yup.object().shape({
  password: Yup.string().max(50, 'Maximum 50 symbols').required('نام کاربری'),
})
...
export function ForgotPass() {
  ...
  const sendCode = () => {
    setResShow(true)
    setChoseWay(true)
    setState('کلمه عبور شما به 00000 ارسال شد')
    setNextShow(true)
    setFaalSazi(false)
    setAcceptShow(false)
    setChoseWay(false)
  ...
  return (
    ...
    >
      ...
      <div className='mb-10 text-center'>
        {/* begin::Title */}
        <h1 className='text-dark mb-3'>فراموشی رمز عبور</h1>
        {/* end::Title */}

        {/* begin::Link */}
        <div className='text-gray-400 fw-bold fs-4'>فراموشی رمز عبور </div>
        {/* end::Link */}
      </div>
      {/* end::Heading */}
      ...
      <div className='mb-10 fv-row' data-kt-password-meter='true'>
        <div className={'row 12'}>
          ...
          <label className='col-form-label col-sm-4'>نام کاربری</label>
        </div>
      </div>
      {/* end::Form group */}
      {choseWay && (
